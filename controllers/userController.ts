...
const getUser = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Validate churches data before sending
    const validChurches = (userChurches || []).filter((church: any) => 
      church && 
      church._id && 
      church.name && 
      church.location
    ...
    if (validChurches.length !== userChurches.length) {
      console.warn(`⚠️ User ${req.userId}: Filtered out ${userChurches.length - validChurches.length} invalid churches`);
    }

    // Prepare user response with churches
    const userResponse = {
      ...user.toObject(),
      // Add validated churches array to response
...
const logoutUser = async (req: AuthRequest, res: Response) => {
  try {
    // Import token blacklist utilities
...
const deleteUserAccount = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Step 5: Invalidate the auth token
