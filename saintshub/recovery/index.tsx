...
const SermonLibrary: React.FC<SermonLibraryProps> = ({ presentPlayBottomSheet }) => {
  ...
  const playBottomSheetRef = useRef<PlayBottomSheetHandle>(null);
  ...
  const sermonsQuery = useSermons(
    ...
    'Audio' // Default to audio format
  );

  const yearFilterQuery = useSermonsByYear(
    selectedLanguage?.code || null,
    yearFilter
  );

  const lengthFilterQuery = useSermonsByLength(
    selectedLanguage?.code || null,
    lengthFilter
  );

  const seriesFilterQuery = useSermonsBySeries(
    selectedLanguage?.code || null,
    seriesFilter
  );

  // Determine which query is active based on filters
  const activeQuery = yearFilter ? yearFilterQuery :
    lengthFilter ? lengthFilterQuery :
      seriesFilter ? seriesFilterQuery :
        sermonsQuery;

  // Extract data, loading state, and error from the active query
  const {
    data: sermonsData,
    isLoading: isLoadingSermons,
    error: sermonsError
  } = activeQuery as { data: SermonsResponse | undefined, isLoading: boolean, error: any };

  // Initialize caching system on mount and prefetch common data
  useEffect(() => {
    if (selectedLanguage?.code) {
      // Prefetch common sermon data
      prefetchCommonSermonData([selectedLanguage.code]);

      // Clear old caches periodically (older than 7 days)
      clearOldSermonCaches();
    }
  }, [selectedLanguage?.code, queryClient]);
  ...
  const sermons = React.useMemo<Sermon[]>(() => {
    ...
    if (sermonsData.d && Array.isArray(sermonsData.d)) {
      ...
      return sermonsData.d.map(sermon => ({
        ...sermon,
    ...
    // Check if sermonsData has a sermons property that is an array
      ...
      // Ensure each sermon has an _id property
      return sermonsData.sermons.map(sermon => ({
        ...sermon,
        _id: sermon._id || sermon.code || `sermon-${Math.random().toString(36).substring(2, 9)}`
      }));
    }
  ...
  const handleSearch = async () => {
    ...
    // Since sermons come from React Query now, we just filter the results client-side

    // Access the sermons array directly from the response
    // Make sure sermons is always an array before filtering
    const sermonsData = sermonsQuery.data?.sermons;
    const sermons = Array.isArray(sermonsData) ? sermonsData : [];

    console.log('Sermons data type:', typeof sermonsData, Array.isArray(sermonsData));
    console.log('Sermons to filter:', sermons);

    // Now we can safely filter the array
    const filteredSermons = sermons.filter(
      (sermon: any) => sermon?.title && typeof sermon.title === 'string' && sermon.title.toLowerCase().includes(searchQuery.toLowerCase())
    );

    // We could eventually implement a server-side search endpoint
    // and add a useSermonsBySearch hook
  ...
  const handleFilterChange = (filterType: FilterType, value: string) => {
    ...
    switch (filterType) {
      case 'year':
        setYearFilter(value);
        // Invalidate the year filter query to trigger a refetch
        if (selectedLanguage?.code) {
          queryClient.invalidateQueries({
            queryKey: ['sermons-by-year', selectedLanguage.code, value]
          });
          console.log('Invalidated year query:', selectedLanguage.code, value);
        }
        break;
      case 'length':
        setLengthFilter(value);
        // Invalidate the length filter query
        if (selectedLanguage?.code) {
          queryClient.invalidateQueries({
            queryKey: ['sermons-by-length', selectedLanguage.code, value]
          });
          console.log('Invalidated length query:', selectedLanguage.code, value);
        }
        break;
      case 'series':
        setSeriesFilter(value);
        // Invalidate the series filter query
        if (selectedLanguage?.code) {
          queryClient.invalidateQueries({
            queryKey: ['sermons-by-series', selectedLanguage.code, value]
          });
          console.log('Invalidated series query:', selectedLanguage.code, value);
        }
        break;
      default:
        break;
  ...
  return (
    <View style={styles.container}>
              ...
              {!selectedLanguage ? (
                <>
                  ...
                  <SafeLanguageSelector
                    ...
                    isLoading={isLoadingLanguages}
                    error={languagesError}
                    searchQuery={languageSearchTerm}
                ...
                <>
                  ...
                  <SermonListWrapper
                    ...
                    presentPlayBottomSheet={presentPlayBottomSheet}
                  />
                </>
              )}
            </View>
      ...
      {/* Main Player Modal - Using the modular MainPlayer component */}
      <MainPlayer
        ...
        onClose={() => {
          ...
          console.log('[SermonLibrary] MainPlayer onClose triggered. isMainPlayerVisible:', false, 'showMiniPlayer:', miniPlayerShouldShow);
        }}
        currentSermon={currentSermon || null}
        isPlaying={isPlaying}
        onTogglePlayback={togglePlayback}
        onOpenPdf={handleOpenPdfNavigation}
...
const SermonLibraryWithAudio = () => {
  // Create a ref for the PlayBottomSheet
  const playBottomSheetRef = useRef<PlayBottomSheetHandle>(null);
  ...
  return (
    <AudioProvider>
      ...
      {/* PlayBottomSheet - Contains both MiniPlayer and FullPlayer */}
      <PlayBottomSheet
