 ...
 * 
 * Extracts userId from a valid JWT token if one is present,
 * but does NOT reject the request if no token or invalid token.
 * Useful for public routes that enrich responses for logged-in users.
 */
    ...
    if (token) {
      ...
      if (!tokenIsBlacklisted) {
        ...
        if (decoded && typeof decoded !== 'string') {
          setUserIdOnRequest(req, decoded.userId);
        }
      }
    }
  } catch {
    // Silently ignore auth errors — this is optional
  }
  next();
};
