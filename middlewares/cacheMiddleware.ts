...
export const cacheMiddleware = (ttl: number = 300) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (cachedResponse) {
      ...
      // Send cached response directly without wrapping
      // If we want to add metadata, we should add it as response headers instead
      // to avoid breaking the response structure (especially for arrays)
    ...
    res.json = function(body: any) {
      // Only cache successful responses (2xx status codes) and valid data
      if (res.statusCode >= 200 && res.statusCode < 300 && body !== null && body !== undefined) {
        cache.set(cacheKey, body, ttl);
        logger.debug('Response cached', { key: cacheKey, ttl, bodyType: typeof body });
      } else if (body === null || body === undefined) {
        logger.warn('Skipping cache for null/undefined response', { key: cacheKey });
      }
