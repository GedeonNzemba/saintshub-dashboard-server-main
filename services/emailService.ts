...
const emailHost = getEnvVariable('EMAIL_HOST', false, 'mail.spacemail.com');
const emailPort = parseInt(getEnvVariable('EMAIL_PORT', true, '465') || '465', 10);
...
const sendEmail = async (to: string, subject: string, html: string, from: string) => {
  ...
  const mailOptions = {
    from: from, // e.g., '"SaintsHub" <support@saintshub.net>'
    to: to,
    subject: subject,
    html: html,
  };
...
export const sendWelcomeEmail = async (user: UserDocument) => {
  ...
  const fromAddress = getEnvVariable('EMAIL_FROM_NOREPLY', true) || '"SaintsHub" <support@saintshub.net>'; // Fallback 'from'
...
export const sendAdminNotificationEmail = async (user: UserDocument, role: string, churchSelection: string) => {
   ...
   const fromAddress = getEnvVariable('EMAIL_FROM_NOREPLY', true) || '"SaintsHub" <support@saintshub.net>';
...
export const sendVerificationEmail = async (user: UserDocument, role: string, churchName: string) => {
  ...
  const fromAddress = getEnvVariable('EMAIL_FROM_ADMIN', true) || '"Saintshub Admin" <support@saintshub.net>';
...
export const sendPasswordResetEmail = async (
  ...
  const fromAddress = getEnvVariable('EMAIL_FROM_NOREPLY', true) || '"SaintsHub Security" <support@saintshub.net>';
...
export const sendSecurityAlertEmail = async (
  ...
  const fromAddress = getEnvVariable('EMAIL_FROM_ADMIN', true) || '"SaintsHub Security" <support@saintshub.net>';
...
/**
 * Send storage upgrade request notification to admin
 */
export const sendStorageUpgradeRequestEmail = async (
  userName: string,
  userEmail: string,
  currentPlan: string,
  ...
  if (!transporter) {
    console.error('Storage upgrade request email not sent: Transporter not available.');
    return;
  }

  const adminEmail = getEnvVariable('ADMIN_NOTIFICATION_EMAIL');
  if (!adminEmail) {
    console.error('ADMIN_NOTIFICATION_EMAIL not set. Cannot send storage upgrade request.');
    return;
  }
  ...
  try {
    const htmlContent = EmailTemplates.storageUpgradeRequestTemplate(
      userName, userEmail, currentPlan, requestedPlan, reason
    );
    await sendEmail(adminEmail, `📦 Storage Upgrade Request from ${userName}`, htmlContent, fromAddress);
    console.log(`✅ Storage upgrade request email sent to admin for user ${userEmail}`);
  } catch (error) {
    console.error(`Failed to send storage upgrade request email for ${userEmail}:`, error);
  }
};

/**
 * Send storage plan updated confirmation to user
 */
export const sendStoragePlanUpdatedEmail = async (
  userEmail: string,
  userName: string,
  oldPlan: string,
  newPlan: string
  ...
  if (!transporter) {
    console.error('Storage plan updated email not sent: Transporter not available.');
    return;
  }
  ...
  try {
    const htmlContent = EmailTemplates.storagePlanUpdatedTemplate(userName, oldPlan, newPlan);
    await sendEmail(userEmail, `Your SaintsHub Storage Plan Has Been Updated 📦`, htmlContent, fromAddress);
    console.log(`✅ Storage plan updated email sent to ${userEmail}`);
  } catch (error) {
    console.error(`Failed to send storage plan updated email to ${userEmail}:`, error);
  }
};

/**
 * Send a storage upgrade rejection notification to a user
 */
export const sendStorageUpgradeRejectedEmail = async (
  userEmail: string,
  userName: string,
  currentPlan: string,
  requestedPlan: string
  ...
  if (!transporter) {
    console.error('Upgrade rejected email not sent: Transporter not available.');
    return;
  }
  ...
  try {
    const htmlContent = EmailTemplates.storageUpgradeRejectedTemplate(userName, currentPlan, requestedPlan);
    await sendEmail(userEmail, `Update on Your Storage Upgrade Request`, htmlContent, fromAddress);
    console.log(`✅ Upgrade rejected email sent to ${userEmail}`);
  } catch (error) {
    console.error(`Failed to send upgrade rejected email to ${userEmail}:`, error);
  }
};

/**
 * Send a custom admin reply email to a user
 */
export const sendAdminReplyEmail = async (
  ...
  if (!transporter) {
    console.error('Admin reply email not sent: Transporter not available.');
    return;
  }
  ...
  try {
    const htmlContent = EmailTemplates.adminReplyTemplate(userName, subject, message);
    await sendEmail(userEmail, subject, htmlContent, fromAddress);
    console.log(`✅ Admin reply email sent to ${userEmail}`);
  } catch (error) {
    console.error(`Failed to send admin reply email to ${userEmail}:`, error);
    throw error; // Re-throw so the controller can report the failure
  }
};
